...
Единый Java-сервис: Spring Boot backend + встроенный frontend мини-игры (React build лежит в `casino-app/src/main/resources/static`). Пользователь входит в комнату, резервирует баллы, при желании покупает буст и дожидается результата раунда. Победитель определяется взвешенным RNG.
