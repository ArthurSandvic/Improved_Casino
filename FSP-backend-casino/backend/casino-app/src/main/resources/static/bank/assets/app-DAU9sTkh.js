...
async function refreshWallet() {
  ...
  try {
    ...
    if (el.filterEntryFee && d.bankFilterEntryFee != null) {
      el.filterEntryFee.value = String(d.bankFilterEntryFee);
    }
    if (el.filterSeats && d.bankFilterSeats != null) {
      el.filterSeats.value = String(d.bankFilterSeats);
    }
    state.config.entryFee = Number(d.bankFilterEntryFee) || state.config.entryFee;
    state.config.seats = Number(d.bankFilterSeats) || state.config.seats;
  } catch {
    /* ignore */
  }
}
